...
{
    internal class VendingMachine : IEnumerable<VendingMachineOption>
    {
        private readonly VendingMachineOption[,] options = new VendingMachineOption[5, 4];
        ...
        public object this[int x, int y] => options[x, y];

        public IEnumerator<VendingMachineOption> GetEnumerator()
        {
            return (IEnumerator<VendingMachineOption>) options.GetEnumerator();
        }
        ...
        public void PrintVendingMachine(IEnumerable<VendingMachineOption> exclusions)
        {
            ...
            for (var row = 0; row < 5; row++)
            {
                ...
                for (var col = 0; col < 4; col++)
                    Console.Write(exclusions.Contains(options[row, col])
                        ? FormattedStringToFifteenCharCentered("X")
                        : FormattedStringToFifteenCharCentered(options[row, col].ToString()));
                Console.Write("|");
                Console.WriteLine();
            }
        ...
        {
            var count = s.Length;
            var amountNeeded = 15 - count;
            var left = amountNeeded / 2;
